...
class AdvocateHelperActivity : AppCompatActivity() {
    ...
    override fun onCreate(state: Bundle?) {
        ...
        findViewById<EditText>(R.id.edtPreviousDate).setOnClickListener { pick(previousDate) }
        findViewById<EditText>(R.id.edtCurrentDate).setOnClickListener { pick(currentDate) }
        findViewById<EditText>(R.id.edtNextDate).setOnClickListener { pick(nextDate) }
    ...
    private fun saveCase() {
        ...
        val item = AdvocateCase(editingId, cn, client, clientPhone.text.toString().trim(), court.text.toString().trim(), previousDate.text.toString().trim(), currentDate.text.toString().trim(), nextDate.text.toString().trim(), currentUpdate.text.toString().trim(), newUpdate.text.toString().trim(), fee, paid, now, now)
    ...
    private fun fill(item: AdvocateCase) {
        editingId = item.id
        caseNumber.setText(item.caseNumber); clientName.setText(item.clientName); clientPhone.setText(item.clientPhone)
        court.setText(item.courtName); previousDate.setText(item.previousDate); currentDate.setText(item.currentDate); nextDate.setText(item.nextDate)
        currentUpdate.setText(item.currentUpdate); newUpdate.setText(item.newUpdate); totalFee.setText(item.totalFee.toString()); received.setText(item.amountReceived.toString())
