...
class SettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        initialiseViews(); loadSettings(); applyRoleAccess(); setupButtons()
    }

    private fun initialiseViews() {
        edtNameColumn = findViewById(R.id.edtNameColumn); edtPhoneColumn = findViewById(R.id.edtPhoneColumn)
        edtDefaultMessage = findViewById(R.id.edtDefaultMessage); edtSmsDelay = findViewById(R.id.edtSmsDelay)
        switchEditMessage = findViewById(R.id.switchEditMessage); switchSkipSent = findViewById(R.id.switchSkipSent)
        switchConfirmSend = findViewById(R.id.switchConfirmSend); switchLogging = findViewById(R.id.switchLogging)
        btnSaveSettings = findViewById(R.id.btnSaveSettings); btnViewLogs = findViewById(R.id.btnViewLogs)
        btnClearLogs = findViewById(R.id.btnClearLogs); btnAuthentication = findViewById(R.id.btnAuthentication)
    }
    ...
    private fun loadSettings() {
        ...
        edtNameColumn.setText(settings.nameColumn); edtPhoneColumn.setText(settings.phoneColumn)
        edtDefaultMessage.setText(settings.defaultMessage); edtSmsDelay.setText(settings.smsDelayMs.toString())
        switchEditMessage.isChecked = settings.editMessageOnScreen; switchSkipSent.isChecked = settings.skipAlreadySent
        switchConfirmSend.isChecked = settings.confirmBeforeBulkSend; switchLogging.isChecked = settings.loggingEnabled
    }
    ...
    private fun setupButtons() {
        ...
        findViewById<Button>(R.id.btnAdvocateDiary).setOnClickListener {
            if (LicenseManager.isFeatureEnabled(this, "advocate_diary")) startActivity(Intent(this, AdvocateDiaryActivity::class.java))
            else Toast.makeText(this, "Advocate Diary is not enabled in this license.", Toast.LENGTH_LONG).show()
        }
        ...
        btnSaveSettings.setOnClickListener {
            ...
            AppSettingsManager.save(this, settings); AppLogger.success(this, "SETTINGS", "Application settings saved")
        ...
        btnClearLogs.setOnClickListener {
            AlertDialog.Builder(this).setTitle("Clear logs?").setMessage("All local application logs will be deleted.")
                .setNegativeButton("CANCEL", null).setPositiveButton("CLEAR") { _, _ -> AppLogger.clear(this); Toast.makeText(this, "Logs cleared", Toast.LENGTH_SHORT).show() }.show()
        }
    }

    private fun showLogs() {
        if (!LicenseManager.isFeatureEnabled(this, "sms_logs") && !SecurityManager.isAdmin(this)) {
            Toast.makeText(this, "Logs are not enabled in this license.", Toast.LENGTH_LONG).show(); return
        }
        val text = AppLogger.read(this)
        val view = TextView(this).apply { this.text = text; textSize = 13f; setPadding(24, 12, 24, 12); setTextIsSelectable(true) }
