...
class BackupRestoreActivity : AppCompatActivity() {
    ...
    private fun requestPassword(title: String, onPassword: (String) -> Unit) {
        ...
        AlertDialog.Builder(this).setTitle(title).setMessage("Use the same password when restoring this .kba file.")
            .setView(input).setNegativeButton("CANCEL", null)
            .setPositiveButton("CONTINUE") { _, _ ->
                val password = input.text.toString()
                if (password.length < 6) Toast.makeText(this, "Password must be at least 6 characters.", Toast.LENGTH_LONG).show()
    ...
    private fun readBackup(uri: Uri, password: String) {
        executor.execute {
            try {
                ...
                runOnUiThread {
                    AlertDialog.Builder(this).setTitle("Confirm Restore")
                        ...
                        .setPositiveButton("RESTORE & MERGE") { _, _ -> performRestore() }.show()
                }
            } catch (_: Exception) {
    ...
    private fun performRestore() {
        ...
        executor.execute {
            try {
                ...
                runOnUiThread { Toast.makeText(this, "Restore complete: $inserted added, $updated updated.", Toast.LENGTH_LONG).show(); refreshInfo() }
            } catch (e: Exception) {
    ...
    override fun onDestroy() { executor.shutdownNow(); if (::db.isInitialized) db.close(); super.onDestroy() }
}
