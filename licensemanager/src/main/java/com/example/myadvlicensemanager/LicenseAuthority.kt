...
/** Offline license issuing authority. The private signing key is encrypted at rest. */
object LicenseAuthority {
    ...
    fun hasKey(c: Context): Boolean = c.getSharedPreferences(PREFS, 0).contains(KEY_CIPHERTEXT)

    /** Local manager role is configured by the owner; no privileged phone numbers are hardcoded. */
    ...
    fun installSigningKey(c: Context, input: InputStream, password: CharArray): Boolean = try {
    ...
    private fun sign(c: Context, payload: String): String? = try {
        ...
        Base64.encodeToString(bytes, Base64.NO_WRAP) + "." + Base64.encodeToString(signature.sign(), Base64.NO_WRAP)
    ...
    private fun decryptKey(c: Context): PrivateKey {
        ...
        val cipher = javax.crypto.Cipher.getInstance("AES/GCM/NoPadding")
        cipher.init(javax.crypto.Cipher.DECRYPT_MODE, wrapKey(), javax.crypto.spec.GCMParameterSpec(128, iv))
